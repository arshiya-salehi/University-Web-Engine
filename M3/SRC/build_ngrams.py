...
class NgramBuilder:
    ...
    def build_ngrams(self, data_dir):
        """Build n-gram index"""
        print("=" * 70)
        print("EXTRA CREDIT #2: N-GRAM INDEXING (BIGRAMS & TRIGRAMS)")
        print("=" * 70)
        ...
        # Print statistics
        stats = self.ngram_indexer.get_ngram_statistics()
        
        print("\n" + "=" * 70)
        print("✓ N-GRAM INDEX COMPLETE!")
        print("=" * 70)
        print(f"\nStatistics:")
        print(f"  - Total bigrams: {stats['total_bigrams']}")
        print(f"  - Total trigrams: {stats['total_trigrams']}")
        print(f"  - Total bigram postings: {stats['total_bigram_postings']}")
        print(f"  - Total trigram postings: {stats['total_trigram_postings']}")
        print(f"  - Bigram index file: {self.ngram_indexer.bigram_file}")
        print(f"  - Trigram index file: {self.ngram_indexer.trigram_file}")


def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python build_ngrams.py <data_directory> [index_directory]")
        sys.exit(1)
    ...
    builder = NgramBuilder(index_dir)
    builder.build_ngrams(data_dir)
